...
extern const unsigned char hey_ardo_tflite[]; 
...
#define WIFI_SSID           "Ardo"          // ← CAMBIAR
#define WIFI_PASSWORD       "ardi2siempre"      // ← CAMBIAR
...
static void init_microphone() {
    ...
    // Forzamos leer el canal izquierdo (L/R a GND) igual que Arduino
...
static bool tcp_recv_tts_to_speaker(int sock) {
    ...
    ESP_LOGI(TAG, "TTS PCM len=%lu bytes", (unsigned long)pcm_len);
...
// ─── Clasificador Local ───────────────────────────────────────────────────────
static local_intent_t classify_local_intent(const int16_t* frames, int n_frames_x_samples) {
    float rms[5] = {};
    ...
    if (n_frames_x_samples > 1500 * I2S_BLOCK_SAMPLES / 160) return LOCAL_INTENT_ROBOT;
...
static void uart_monitor_task(void* arg) {
    ...
    while (true) {
        int len = uart_read_bytes(UART_PORT, rx_buf, sizeof(rx_buf) - 1, pdMS_TO_TICKS(50));
        if (len <= 0) continue;
        rx_buf[len] = '\0';
...
// ─── Tarea: I2S Reader (Clon exacto de la lógica de Arduino) ─────────────────
static void i2s_reader_task(void* arg) {
    int32_t* raw32 = (int32_t*)heap_caps_malloc(I2S_BLOCK_SAMPLES * sizeof(int32_t), MALLOC_CAP_INTERNAL);
    ...
    while (true) {
        size_t br;
        if (i2s_channel_read(s_rx_chan, raw32, I2S_BLOCK_SAMPLES * 4, &br, portMAX_DELAY) != ESP_OK) continue;
        int n = br / 4;
        
        double rms_acc = 0;

        for (int i = 0; i < n; i++) {
            // 👇 LA MAGIA: Exactamente igual que en tu script de Arduino (* 12)
            int32_t s = (raw32[i] >> 16) * 60;
           
            // Constrain manual (Límite de 16 bits)
            if (s > 32767) s = 32767;
            if (s < -32768) s = -32768;
            
            pcm16[i] = (int16_t)s;
            rms_acc += ((double)s * (double)s); // Para calcular la energía
        }

        // Medidor de volumen cada ~500ms
        if (++print_counter >= 50) { 
            int rms = (int)sqrt(rms_acc / n);
            ESP_LOGI(TAG, "🎤 Nivel de volumen (RMS): %d", rms);
            print_counter = 0;
        }
...
// ─── Tarea Principal: Wake Word + Pipeline ───────────────────────────────────
static void wakeword_task(void* arg) {
    ...
    tflite::MicroAllocator* alloc = tflite::MicroAllocator::Create(s_tensor_arena, TENSOR_ARENA_SIZE);
    ...
    int16_t* capture_buf = (int16_t*)heap_caps_malloc(MAX_CAPTURE_SAMPLES * sizeof(int16_t), MALLOC_CAP_SPIRAM);
    int        capture_idx = 0;
    ...
    while (true) {
        ...
        // ── 1. BYPASS DEL AFE (Procesamos RAW PCM directo al modelo para la Wake Word) ──
        if (s_state == STATE_IDLE) {
            size_t used;
            struct FrontendOutput fe = FrontendProcessSamples(&s_fe_state, pcm_block, I2S_BLOCK_SAMPLES, &used);
            
            if (fe.values && fe.size == NUM_MEL_CHANNELS) {
                ...
                memmove(spec_history, spec_history + NUM_MEL_CHANNELS, inp->bytes - NUM_MEL_CHANNELS);
                ...
                if (interp.Invoke() == kTfLiteOk) {
                    ...
                    if (++log_counter >= 10) { 
                        ESP_LOGI(TAG, "Prob: %.3f | Promedio: %.3f", prob, avg);
                        log_counter = 0;
                    }
                    ...
                    if (avg >= PROBABILITY_CUTOFF) {
                        ...
                        led_set(0, 200, 0);
                        xSemaphoreTake(s_state_mux, portMAX_DELAY);
                        s_state = s_turbo_mode ? STATE_TURBO_STREAM : STATE_LOCAL_CAPTURE;
                        xSemaphoreGive(s_state_mux);
                        sw_reset(&sw);
                        capture_idx = 0;
                        afe_written = 0; // Preparamos el AFE para grabar
                    }
                }
            }
        }

        // ── 2. TURBO MODE: Streaming al servidor (Aquí sí usamos el AFE limpio) ──
        if (s_state == STATE_TURBO_STREAM) {
            ...
            if (sock < 0) {
                ...
                continue; // Saltamos de inmediato al fallback
            }

            send(sock, TCP_HEADER, 9, 0); // Header
            ...
            while (!stream_done && total_frames < CMD_CAPTURE_FRAMES * 2) {
                if (xQueueReceive(s_i2s_queue, pcm_block, pdMS_TO_TICKS(100)) != pdTRUE) break;

                // Alimentamos el AFE con el audio que va llegando
                ...
                while (rem2 > 0) {
                    ...
                    // Enviamos audio procesado a la PC
            ...
            send(sock, TCP_FOOTER, 9, 0); // Footer
            ESP_LOGI(TAG, "Stream enviado (%d frames) — esperando TTS...", total_frames);
            led_set(200, 150, 0);  // amarillo = esperando
            ...
            if (tts_ok) {
                ...
                xSemaphoreGive(s_state_mux);
                led_set(0, 200, 200);  // cyan = play
                xEventGroupWaitBits(s_evt, EVT_SPK_DONE, pdTRUE, pdTRUE, pdMS_TO_TICKS(30000));
            }
            goto enter_cooldown;
        }

        // ── 3. MODO LOCAL: Guardar en memoria (También usamos AFE limpio) ──
        if (s_state == STATE_LOCAL_CAPTURE) {
            led_set(200, 80, 0);  // naranja = local
            int silence_cnt = 0;
...
capture_done:
            ...
            const char* cmd_text = (intent < LOCAL_INTENT_UNKNOWN) ? LOCAL_INTENT_CMDS[intent] : "comando desconocido";
            ...
            EventBits_t bits = xEventGroupWaitBits(s_evt, EVT_UART_RESP, pdTRUE, pdTRUE, pdMS_TO_TICKS(UART_RESP_TIMEOUT_MS));
...
extern "C" void app_main() {
    ...
    if (!s_tensor_arena) s_tensor_arena = (uint8_t*)heap_caps_malloc(TENSOR_ARENA_SIZE, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
